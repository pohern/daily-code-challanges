...
function formatWithPadding(num, pad, len) {
  return num.toString().padStart(len, pad);
}
...
function mapArray(arr,callb){
  let newArr = []
  arr.forEach(function(el, idx){
    newArr.push(callb(el, idx))
  })
  return newArr
}
...
function reduceArray(arr, cb, ivAcc){
  let acc = ivAcc
  arr.forEach(function(el, idx){
    acc = cb(acc, el, idx)
  })
...
// This function, given a non-empty array A or N integers(sorted in non-decrementing order) and integer K, checks whether A contains numbers 1,2,...,K(every number from 1 to K at least once) and no other numbers  
// EX A[0] = 1,A[1] = 1, A[2]=2,A[3]=3,A[4]=3 K=3 would return true
//EX A[0] = 1, A[1] = 1, A[2] = 3 K=2 would return false
//Assume that 
//N and K are integers within the range [1,...,300000]
//Each element of array A is an integer within the range [0,...,1000000]
//Array A is sorted in  anon-decrementing order.
...
//     return True
// }
//Modify two lines at most to make this function work.

// You have a long piece of text, and you want to send it to your friend as a series of SMS messages. Your text consists of english letters(lowercase & uppercase) and PerformanceObserver. The text doesn't contain any leading spaces at the beginning or trailing spaces at the end. There are no two consecutive spaces in the text. One proper SMS can contain K letters. IF the text is longer, it needs to be split into parts. Each part should be sent in a separate message. Text may not be split within words. The order of the messages can't change, so that we can later concatenate the resulting message. Spaces at the end of messages are removed, and not accounted for in the overall message length. The goal is to count the number of SMS messages needed to accommodate the whole text, keeping the length of each message less than or equal to K. The total number of messages must be kept to a minimum, however it would not be acceptable, for example, to the wasteful approach of sending one word per message.
// function solution(S, K):
// S: the text needed to be split, K: Max number of characters in the SMS
// EX: S:'SMS messages are really short' K = 12, should return 3
// "SMS messages", 'are really', 'short'
// IF it is impossible to split, your function should return -1
// Assume: K is in the range [1,...500], S is a non-empty string of at most 500 characters

// An array A consisting of N integers is given. The elements of array A together represent a chain, and each element represents the strength of each link in the chain. We want to divide the chain into three smaller chains. All we can do is break the chain in exactly two non-adjacent positions. More persicesly, we should break links P,Q(0<P<Q<n-1, Q-P > 1), resulting in three chains [0, P-1] [P+1, Q-1] [Q+1, N-1]. The total cost of this operation is equal to A[P] + A[Q].
//EX 
//A[0] = 5
//A[1] = 2
//A[2] = 4
...
//(1, 3): Total Cost 2+6=8
//(1, 4): Total Cost 2+3=5 Lowest cost so we would break here
//(2,4): Total Cost 4+3=7
//Write a function, that given an array of N integers, returns the minimal cost of dividing the chain into three pieces.
